  ...
  const handleScan = useCallback(
    (detectedCodes: IDetectedBarcode[]) => {
      ...
      setTimeout(async () => {
        ...
        if (apiResult) {
          // Store real response for the result page
          sessionStorage.setItem("qr_scan_result", JSON.stringify(apiResult));
          const verdict = apiResult.risk.verdict === "dangerous" ? "danger" : apiResult.risk.verdict;
          router.push(`/result?verdict=${verdict}`);
        } else {
          // Backend unreachable — use simple heuristic, no stored result
          sessionStorage.removeItem("qr_scan_result");
          const verdict = url.includes("paypal.com") && !url.includes("paypa1") ? "safe" : "danger";
          router.push(`/result?verdict=${verdict}`);
        }
      }, 800);
    },
